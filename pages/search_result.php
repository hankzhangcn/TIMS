...
if($_POST['t_name'])
    $t_name =" AND t_name LIKE '%".$t_name."%'";
if($_POST['t_gender'])
    $t_gender =" AND t_gender = '".$t_gender."'";
if($_POST['t_dep'])
    $t_dep =" AND t_dep = '".$t_dep."'";
if($_POST['t_edu'])
    $t_edu =" AND t_edu LIKE '%".$t_edu."%'";
if($_POST['t_title'])
    $t_title =" AND t_title LIKE '%".$t_title."%'";
if($_POST['t_telep'])
    $t_telep =" AND t_telep LIKE '%".$t_telep."%'";
if($_POST['t_statu'] != NULL)
    $t_statu =" AND t_statu = ".$t_statu;

$sql = "SELECT * FROM t_info WHERE t_id IS NOT NULL ".$t_name.$t_gender.$t_dep.$t_edu.$t_title.$t_telep.$t_statu." ORDER BY t_id";
echo $sql;
                ...
                <body>
                    <main>
                        ...
                        <div class="contentflow">
                            <div class="noticepad">
                                <h1></h1>
                                <a>管理员 <?php echo $_SESSION['u_id'];?>，这是您要找的教师列表。</a></br>
                                <a>要管理教师信息，请点击教师姓名。</a></br>
                                <a>要登出系统，请返回仪表盘。</a></br>
                            </div>
                            ...
                            <div class="workspace">
                                ...
                                <table class="glance" >
                                    ...
                                    <tr>
                                        <td>
                                            <a href="menage.php?t_id=<?php echo $row['t_id']?>"><?php echo $row['t_id']?></a>
                                        </td>
                                        <td>
                                            <a href="menage.php?t_id=<?php echo $row['t_id']?>"><a href="menage.php?t_id=<?php echo $row['t_id']?>"><?php echo $row['t_name']?></a>
                                        </td>
                                        <td>
                                            <?php echo $row['t_gender']?>
                                        </td>
                                        <td>
                                            <?php echo $row['t_birthday']?>
                                        </td>
                                        <td>
                                            <?php echo did_dname($row['t_dep']);?>
                                        </td>
                                        <td>
                                            <?php echo $row['t_edu']?>
                                        </td>
